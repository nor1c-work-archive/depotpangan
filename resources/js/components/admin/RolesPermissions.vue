                  ...
                  <div class="form-group row">
                    <div class="col-md-12">
                      <fieldset class="form-group mb-3">
                        <input type="text" v-model="role_name" class="form-control round bg-transparent text-dark"
              ...
              <div class="card-body">
                <ul class="nav flex-column nav-pills mb-3" id="v-pills-tab" role="tablist" aria-orientation="vertical">
                  <li class="nav-item" v-for="permission in permissions" v-if="permission.parent_id == '0'">
                    <a class="nav-link" :id="permission.value + '-a-tab'" data-toggle="pill"
                      :href="'#' + permission.value + '-a'" role="tab" :aria-controls="permission.value + '-a'"
                      aria-selected="false">{{ permission.name }}</a>
                  </li>
                </ul>
              </div>
            </div>
            ...
            <div class="card card-custom gutter-b bg-white border-0 roles-main">
              <div class="card-body">
                <div class="tab-content" id="v-pills-tabContent">
                  <div v-for="permission in permissions" class="tab-pane fade" :id="permission.value + '-a'"
                    role="tabpanel" :aria-labelledby="permission.value + '-a-tab'" v-if="permission.parent_id == '0'">
                    ...
                    <div class="form-group row">
                      <div class="col-12">
                        <div class="p-2 bg-light mb-4">
                          <div class="switch-h d-flex justify-content-between mb-0">
                            ...
                            <div class="
                                ...
                                mr-0
                              ">
                              <input type="checkbox" class="custom-control-input" :id="'customdashboard' + permission.id"
                                :value="permission.id" @change="check($event)"
                                v-bind:checked="selectedPermission.includes(permission.id) ? 'checked' : ''" />
                              <label class="custom-control-label mr-1" :for="'customdashboard' + permission.id">
                              </label>
                            </div>
                          </div>
                        </div>
                        <div class="permissionsallow0" v-for="childpermission in permissions"
                          v-if="permission.id == childpermission.parent_id">
                          <div class="toggle-swither px-5 py-3 border-top"
                            v-if="selectedPermission.includes(permission.id)">
                            <div class="
                                switch-h
                                d-flex
                              ...
                              <div class="
                                ...
                                <input type="checkbox" class="custom-control-input"
                                  :id="'customView' + childpermission.id" :value="childpermission.id"
                                  @change="check($event)"
                                  v-bind:checked="selectedPermission.includes(childpermission.id) ? 'checked' : ''" />
                                <label class="custom-control-label mr-1" :for="'customView' + childpermission.id">
                                </label>
                              </div>
                            </div>
                          </div>
